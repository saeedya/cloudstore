...
class TestAuthController:
    ...
    def test_reset_password(self, auth_controller, db_session):
        ...
        try:
            ...
            assert created_user.reset_token_expires is not None, "Reset token expiry not set"
            
            # Log timezone information
            logging.info(f"Stored token expiry: {created_user.reset_token_expires}")
            logging.info(f"Token expiry timezone info: {created_user.reset_token_expires.tzinfo}")
            ...
            assert status_code == 200, f"Expected status 200, got {status_code}. Response: {response}"
            
            # Verify changes
            updated_user = User.query.filter_by(username='testuser').first()
            assert updated_user.check_password('NewPass123@'), "Password was not updated"
            assert updated_user.reset_token is None, "Reset token was not cleared"
            assert updated_user.reset_token_expires is None, "Reset token expiry was not cleared"
